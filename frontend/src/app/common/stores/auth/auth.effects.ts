        ...
        return actions$.pipe(
            ...
            switchMap(() =>
                accountService.me().pipe(
                    ...
                    catchError((error) => {
                        console.error(error);
                        toast.error('Could not load account details, you were logged off from the application! Please try again later or contact the site\'s administrator.');
                        return of(AuthActions.accountLoadedError()).pipe(
