...
@Injectable()
export class SearchJobsStore extends ComponentStore<SearchJobsState> {
    private list$ = this.select((state) => state.list);

    searchTerm$ = this.select((state) => state.searchTerm);
    vm$ = this.select({
        list: this.select(this.list$, this.searchTerm$, this.extractFilteredList.bind(this)),
    ...
    private getListLoadedSuccess = this.updater(
        (state, list: JobList): SearchJobsState => ({
            ...state,
            list,
            loading: false
        })
